...
class CsHttpServerExtension {
	@Inject
	extension AttributeExtension

	def String controllerNamespace(HttpServer it) '''«getName.substring(0, name.lastIndexOf('.'))»'''
	
	def String namespace(HttpServer it) '''«name»'''

	def String fileExtension() '''.cs'''

	def String controllerName(HttpServer it) '''«getName.substring(name.lastIndexOf('.')+1).toFirstUpper»Controller'''

	def String namespaceFolder(HttpServer it) '''«namespace.replace('.', '/')»'''
